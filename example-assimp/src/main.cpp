...
class ofApp : public ofBaseApp {
	public:
        ...
        void setup(){
            ...
            model  = assimp.getMesh(0);
            float scale = 4.5;
            for( int i = 0; i < model.getNumVertices(); i++ ) {
                 model.getVertices()[i] = model.getVertices()[i] * scale; //scale mesh assimp
            }
            ...
            ofxReflectionRefraction r1,r2,r3;
            r1.setup(sphere,pos,neg,true); // true draw background cubemap
            Ref.push_back(r1);
            r2.setup(box,pos,neg,false);
            Ref.push_back(r2);
            r3.setup(model,pos,neg,false);
            Ref.push_back(r3);
        ...
        void draw(){
            ...
            Ref[0].drawMeshGlass(cam,ofVec3f(-100,0,0));
            Ref[1].drawMeshGlass(cam,ofVec3f(100,0,0));          
            Ref[2].drawMeshGlass(cam,translate.get());
